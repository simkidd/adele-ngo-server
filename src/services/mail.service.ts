...
const send = async (options: MailOptions): Promise<boolean> => {
  try {
    ...
    logger.error(`Email failed to ${options.to}: ${(error as Error).message}`);
...
<body>
  <div class="container">
    ...
    <div class="footer">
      <p>${config.foundation.NAME} &nbsp;|&nbsp; ${config.foundation.WEBSITE}</p>
...
export const sendApplicationConfirmation = async (
  ...
  await send({
    ...
    html: baseLayout(`
      ...
      <a href="${config.foundation.WEBSITE}/login" class="btn">Check Your Status</a>
...
export const sendApplicationAccepted = async (
  ...
  await send({
    ...
    html: baseLayout(`
      ...
      <a href="${config.foundation.WEBSITE}/dashboard" class="btn">View Dashboard</a>
    `),
  });
};

// ── Application enrolled ──────────────────────────────────────────────────────
export const sendApplicationEnrolled = async (
  to: string,
  name: string,
  programTitle: string,
  cohortName: string,
...
export const sendCertificateIssued = async (
  ...
  await send({
    ...
    html: baseLayout(`
      ...
      <a href="${config.foundation.WEBSITE}/verify/${certId}" style="display:inline-block;margin-left:12px;color:#F97316;font-weight:bold;">Verify Online →</a>
...
export const sendAdminRegistrationAlert = async (
  adminEmail: string,
  ...
  await send({
    ...
    html: baseLayout(`
      ...
      <a href="${config.app.ADMIN_URL}/programs/registrations" class="btn">Review Application</a>
    `),
  });
};
...
export const sendContactConfirmation = async (
  ...
  await send({
    ...
    html: baseLayout(`
      ...
      <a href="${config.foundation.WEBSITE}" class="btn">Visit Our Website</a>
    `),
  });
};
